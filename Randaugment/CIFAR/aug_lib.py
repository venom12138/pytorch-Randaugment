...
def _contrast_impl(img, v):  # [0.1,1.9]
    v = (float(v) / 30) * 0.9
    assert 0 <= v <= 0.9
    if random.random() > 0.5:
        v = -v
...
contrast = TransformT('Contrast', _contrast_impl)

def _color_impl(img, v):  # [0.1,1.9]
    v = (float(v) / 30) * 0.9
    assert 0 <= v <= 0.9
    if random.random() > 0.5:
        v = -v
...
color = TransformT('Color', _color_impl)

ohl_color = TransformT('Color', _enhancer_impl(ImageEnhance.Color, .3, .9))

def _brightness_impl(img, v):  # [0.1,1.9]
    v = (float(v) / 30) * 0.9
    assert 0 <= v <= 0.9
    if random.random() > 0.5:
        v = -v
...
brightness = TransformT('Brightness', _brightness_impl)

def _sharpness_impl(img, v):  # [0.1,1.9]
    v = (float(v) / 30) * 0.9
    assert 0 <= v <= 0.9
    if random.random() > 0.5:
        v = -v
...
sharpness = TransformT('Sharpness', _sharpness_impl)
...
class RandAugment:
    def __init__(self, n = 2, m = 10, Res = False):
        self.n = n
        self.m = m  # [0, 30]
        self.Res = Res # True or False
    def __call__(self, img):
        ...
        if self.Res:
            ...
            for op in ops:
                ...
                if random.random() < prob:
                    img_after = op.pil_transformer(1., self.m)(img)
